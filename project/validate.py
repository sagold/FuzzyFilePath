...
"""
class Validate:
    # Helper object used for validating directories of current file/view
    ...
    def view(view, config, open_dialog=False):
        ...
        """
        if (Validate.file_has_location(view) is False):
            return show_dialog("disabled: file is temporary", open_dialog)

        if (Validate.is_project() is False):
            return show_dialog("disabled: not a project", open_dialog)

        directory = Validate.project_directory(view, config["PROJECT_DIRECTORY"])
        if directory is False:
            return show_dialog("\ndisabled:{0}<div>not within project directory {1}"
                    .format(view.file_name(), config["PROJECT_DIRECTORY"]), open_dialog)

        config["BASE_DIRECTORY"] = Validate.sanitize_base_directory(config["BASE_DIRECTORY"], directory["project"], directory["base"])
        directory["current"] = Validate.get_current_folder_relative(view, directory["project"])
    ...
    def project_directory(view, project_directory):
        ...
        if project_directory:
            ...
            directory["settings"] = Validate.get_valid_path(project_directory)
